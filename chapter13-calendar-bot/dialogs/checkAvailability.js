...
lib.dialog(constants.dialogNames.CheckAvailability, [
    (session, args) => {
        chatbase.build(session.message.text, session.message.address.user.id, args).send();
    ...
    (session, results, next) => {
        ...
        if (!entry.hasDateTime && !entry.hasRange) {
            chatbase.build(session.message.text, session.message.address.user.id, null, true).send();
